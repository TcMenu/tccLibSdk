...
namespace tcremote {
    ...
    public:
        explicit MbedTlsEncryptionHandler(const uint8_t* keyIn, const uint8_t *initialIv);
        void initialise();
        int encryptData(const uint8_t *plainText, int bytesIn, uint8_t *buffer, size_t buffLen) override;
